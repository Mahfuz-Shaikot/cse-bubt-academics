# 🎓 CSE BUBT Academics
A comprehensive tracking repository of my software engineering coursework, system implementations, and algorithmic problem-solving at BUBT.

# **`Table of Content`**

- **Academic Terms**
  - [**Year 1** | *Spring 2024*](#year-1--spring-2024)
  - [**Year 1** | *Fall 2024*](#year-1--fall-2024)
  - [**Year 2** | *Spring 2025*](#year-2--spring-2025)
  - [**Year 2** | *Fall 2025*](#year-2--fall-2025)
  - [**Year 3** | *Spring 2026*](#year-3--spring-2026)
  - [**Year 3** | *Summer 2026*](#year-3--summer-2026)

- **Term Projects**
  - [*`FIRS`*](https://github.com/Mahfuz-Shaikot/FIRS) *(Spring 2026)*
  - [*`DealerSync`*](https://github.com/Mahfuz-Shaikot/DealerSync) *(Summer 2026)*
  - [*`ACSIS`*](https://github.com/Mahfuz-Shaikot/Arduino-Air-Conditioner-Safety-System-ACSIS) *(Summer 2026)*
  - [*`CSAPM`*](https://github.com/Mahfuz-Shaikot/Chocolate-Sales-Analytics-and-Predictive-Modeling-CSAPM) *(Summer 2026)*

---

## Year 1 | Spring 2024
### CSE 102: Structured Programming Language
- Syntax, Variables & Data Types
- Control Flow & Loops
- Arrays & Strings
- Functions & Pointers

## Year 1 | Fall 2024
### CSE 112: Object Oriented Programming
- Classes, Objects & Encapsulation
- Inheritance & Polymorphism
- Exception Handling & File I/O

### CSE 100: Software Development I
- Project: Initial SDLC Application

### EEE 102: Electrical Technology
- Basic Circuit Configurations
- Voltage & Current Laws

---

## Year 2 | Spring 2025
### CSE 222: Data Structure
- Dynamic Arrays & Linked Lists
- Stacks & Queues
- Trees & Graph Representations

### CSE 206: Digital Logic Design
- Logic Gates & Boolean Algebra
- Combinational Circuits
- Sequential Circuits & Flip-Flops

### EEE 212: Electronic Devices and Circuits
- Diodes & Rectifiers
- Transistors & Amplifiers

## Year 2 | Fall 2025
### CSE 232: Algorithms
- Sorting & Searching Algorithms
- Divide and Conquer
- Greedy Methods & Dynamic Programming
- Graph Algorithms: BFS, DFS, Shortest Path

### CSE 208: Database Systems
- ER Modeling & Schema Design
- Basic & Advanced SQL Queries
- Joins, Subqueries & Aggregations
- PL/pgSQL Triggers & Procedures

### CSE 210: Operating Systems
- Shell Scripting Basics
- Process Management & Scheduling
- Threads & Synchronization
- Memory Management

### CSE 215: Computer Architecture
- Assignment: Instruction Set Architecture & Pipelining

### CSE 200: Software Development II
- Project: Intermediate Database-Backed Application

---

## Year 3 | Spring 2026
### CSE 342: Advanced Programming
- Advanced Data Handling
- Multithreading & Concurrency
- Network Programming / Sockets

### CSE 318: System Analysis and Design
- Requirement Engineering
- UML Modeling: Use Case & Class Diagrams
- System Architecture Mapping

### CSE 320: Computer Networks
- Packet Tracing & Wireshark
- IP Addressing & Subnetting
- Routing Protocols

### CSE 302: Technical Writing and Presentation
- Assignment: Software Requirement Specification (SRS) Documentation

## Year 3 | Summer 2026
### CSE 322: Artificial Intelligence and Expert System
- Search Algorithms: BFS, DFS, A*
- Constraint Satisfaction Problems
- Minimax & Game Trees

### CSE 324: Compiler Design
- Lexical Analysis & Tokenization
- Parsing & Syntax Trees
- Intermediate Code Generation

### CSE 326: Microprocessor and Microcontroller
- Assembly Language Basics (8086)
- Interrupts & I/O Interfacing
- Microcontroller Programming

### CSE 300: Software Development III
- Project: Advanced System / Microservices Integration
